...
```
Axial Resistance:
    Has no effect on $R_i$ nor $\tau_m$ because we have one-compartment model.
    The axial resistance becomes influential when..
...
Capacitance:
    ...
    By increasing the capacitance it now takes longer for the capacitor to charge and as a result the time constant is longer because the current flows slower whilst the capacitor is charging.

Passive conductances/$R_m$:
    Increasing the passive conductances decreases the membrane resistance (because R = 1/g) and as mentioned earlier $R_m$ directly affects $R_i$.
...
Instead it is brought down to -20 mV most likely due to potassium leak conductances and inactivation of sodium channels.
...
When we record the voltage at the dendrite as the alphasynapse is activated we observe that the size of the voltage change is higher than when the synapse was near the soma.
The high voltage change is due to the dendrites smaller size and thus higher input resistance.
